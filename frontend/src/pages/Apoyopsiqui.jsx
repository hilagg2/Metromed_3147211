...
const TABS = [
    { id: 'lineas',  icon: 'fa-phone-alt',      label: 'Líneas de Apoyo' },
    { id: 'faq',     icon: 'fa-question-circle', label: 'Preguntas Frecuentes' },
    { id: 'reporte', icon: 'fa-bullhorn',         label: 'Dar Sugerencia' },
];

const Apoyopsiqui = () => {
    const [showChatbot, setShowChatbot] = useState(false);
    const [activeTab, setActiveTab]     = useState(null); // null = pantalla de inicio
        ...
        return (
            <div className="apoyo-container fade-in">
                <div className="chatbot-wrapper animate-fadeIn" style={{ marginTop: '1rem' }}>
                    <button className="apoyo-btn" onClick={() => setShowChatbot(false)} style={{ marginBottom: '1rem' }}>
                        <i className="fas fa-arrow-left" /> Volver a Recursos de Apoyo
                    </button>
    ...
    return (
        <div className="apoyo-container fade-in">
            ...
            <div className="apoyo-header">
                <h2><i className="fas fa-heartbeat" /> Tu Salud Mental Importa</h2>
                <p>En el Metro de Medellín nos preocupamos por ti. Elige una opción para comenzar.</p>
                <button className="apoyo-btn" style={{ marginTop: '1rem' }} onClick={() => setShowChatbot(true)}>
                    <i className="fas fa-comments" /> Iniciar Chat de Escucha Activa
                </button>
            </div>
                ...
                {TABS.map(tab => (
                    <button
                        ...
                        style={{
                            ...
                            padding: '0.6rem 1.2rem', borderRadius: '30px', cursor: 'pointer',
                            fontWeight: 600, fontSize: '0.95rem', transition: 'all 0.2s',
                            background: activeTab === tab.id ? 'rgba(0,255,136,0.15)' : 'rgba(255,255,255,0.04)',
                            border: activeTab === tab.id ? '1px solid #00ff88' : '1px solid rgba(255,255,255,0.08)',
                        ...
                        {lineas.map((linea) => {
                            ...
                            return (
                                <div className="apoyo-card" key={linea.id_linea} style={{ alignItems: 'flex-start', textAlign: 'left', position: 'relative' }}>
                                    ...
                                    <div className="apoyo-icon"><i className="fas fa-phone-volume" /></div>
                                ...
                                <div key={faq.id_faq} style={{
                                    ...
                                    <button
                                        ...
                                        style={{
                                            ...
                                            padding: '1.25rem', color: '#fff', textAlign: 'left',
                                            fontSize: '1.1rem', fontWeight: '700', cursor: 'pointer',
                                            display: 'flex', justifyContent: 'space-between', alignItems: 'center'
                                        }}
                                    >
                                        <span>{faq.pregunta}</span>
                                        <i className={`fas fa-chevron-${isOpen ? 'up' : 'down'}`} style={{ color: '#00ff88' }} />
                                    </button>
                                    {isOpen && (
                                        <div style={{ padding: '0 1.25rem 1.25rem', color: '#cbd5e1', fontSize: '0.95rem', lineHeight: '1.6', borderTop: '1px solid rgba(255,255,255,0.02)', paddingTop: '1rem' }}>
                                            {faq.respuesta}
                                        </div>
                                    )}
                                </div>
            ...
            {/* ── Sección: Reporte / Sugerencia ─────────────────────── */}
            {activeTab === 'reporte' && (
                <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '20px', padding: '2.5rem' }}>
                    ...
                    <form onSubmit={handleReportSubmit}>
                        <textarea
                            ...
                            style={{
                                ...
                                color: '#fff', padding: '1rem', fontSize: '1rem',
                                fontFamily: 'inherit', marginBottom: '1rem', resize: 'vertical'
                            }}
                            required
                        />
                        {submitSuccess && <p style={{ color: '#00ff88', fontWeight: 'bold', marginBottom: '1rem' }}><i className="fas fa-check-circle" /> {submitSuccess}</p>}
                        {submitError   && <p style={{ color: '#ef4444', fontWeight: 'bold', marginBottom: '1rem' }}><i className="fas fa-exclamation-triangle" /> {submitError}</p>}
                        <button type="submit" className="apoyo-btn" disabled={submitting || !reportText.trim()}>
            ...
            {/* ── Banner de Emergencia — siempre visible ─────────────── */}
            <div className="emergency-banner" style={{ marginTop: '3rem' }}>
                ...
                <a href="tel:123" className="emergency-banner-btn"><i className="fas fa-ambulance" /> Llamar al 123</a>
            </div>
        </div>
    );
};
